"""Simple HR Management MCP Server — protected by HoneyMCP.

Same server as demo_video_before.py, but with ONE line added:
HoneyMCP dynamically generates honeypot tools that fit the HR domain.

Usage:
    uv run python examples/demo_video_after.py
...
mcp = honeypot(mcp, use_dynamic_tools=True, num_dynamic_tools=4, fallback_to_static=True)
# ============================


if __name__ == "__main__":
    print("HR Manager - MCP Server (protected by HoneyMCP)")
    print("=" * 50)
    print("Real tools: search_employees_by_department, get_employee_profile, submit_leave_request, list_departments")
    print("+ Dynamically generated honeypot tools (context-aware)")
    print()
    print("Connect MCP Inspector to: http://localhost:8001/sse")
    print()
