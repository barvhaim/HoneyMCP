"""Watsonx-backed LLM client used by HoneyMCP."""
...
class LLMClient:  # pylint: disable=too-few-public-methods
    """Thin wrapper around LiteLLM completion for HoneyMCP."""
    ...
    def __init__(self, model_full_name=None):
        if model_full_name:
            ...
            self.model = (
                os.getenv("LLM_PROVIDER", "watsonx")
                + "/"
    ...
    def generate_response(self, messages, **kwargs):
        ...
        max__tokens_llm_defined = min(max__tokens_llm_defined, watsonx_max_tokens)
        ...
        try:
            ...
            raise RuntimeError(f"Error generating LLM response: {e}") from e
