...
class AttackerProfiler:
    ...
    def _calculate_sophistication(
        ...
        high_threat_count = sum(1 for e in events if e.threat_level in ["high", "critical"])
    ...
    async def generate_hint(
        ...
        hint = ToolGenerationHint(
            ...
            sophistication_level=(
                "high"
                if profile.sophistication_score > 0.7
                else "medium" if profile.sophistication_score > 0.4 else "low"
            ),
    ...
    async def get_statistics(self) -> Dict[str, any]:
        ...
        return {
            ...
            "automation_detected": len([p for p in self._profiles.values() if p.uses_automation]),
            "avg_tool_diversity": sum(p.tool_diversity for p in self._profiles.values())
            / len(self._profiles),
        }
    ...
    async def export_profiles(self) -> List[Dict[str, any]]:
        ...
        """
        return [profile.model_dump(mode="json") for profile in self._profiles.values()]
