...
class AttackTimeline(BaseModel):
    ...
    # Attacker behavior
    avg_time_between_events: float = Field(
        description="Average time between events in seconds"
    )
...
class ForensicReport(BaseModel):
    ...
    indicators_of_compromise: List[str] = Field(
        description="IOCs extracted from attack"
    )
    ...
    mitigation_steps: List[str] = Field(
        description="Specific mitigation actions"
    )
    ...
    analyst_notes: Optional[str] = Field(
        default=None,
        description="Additional analyst notes"
    )
    tags: List[str] = Field(
        default_factory=list,
...
class ReplayControl(BaseModel):
    ...
    action: str = Field(
        description="Control action: play, pause, stop, seek, speed"
    )
    
    # For seek action
    target_index: Optional[int] = Field(
        default=None,
        description="Target event index for seek"
    )
    
    # For speed action
    speed: Optional[ReplaySpeed] = Field(
        default=None,
...
class ReplayState(BaseModel):
    ...
    current_event: Optional[TimelineEvent] = Field(
        default=None,
        description="Current event being displayed"
    )
    ...
    elapsed_time: float = Field(
        description="Elapsed time in replay (seconds)"
    )
    remaining_time: float = Field(
        description="Remaining time in replay (seconds)"
    )
...
class ComparisonReport(BaseModel):
    ...
    # Differences
    unique_tools_per_session: Dict[str, List[str]] = Field(
        description="Unique tools per session"
    )
    ...
    sophistication_scores: Dict[str, float] = Field(
        description="Sophistication score per session"
    )
