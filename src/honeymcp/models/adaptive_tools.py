...
class ToolEffectivenessMetric(BaseModel):
    ...
    avg_time_to_trigger: float = Field(
        default=0.0,
        description="Average time from session start to first trigger (seconds)"
    )
    total_engagement_time: float = Field(
        default=0.0,
        description="Total time attackers spent with this tool (seconds)"
    )
    
    # Threat detection
    high_threat_triggers: int = Field(
        default=0,
        description="Triggers that resulted in high/critical threat level"
    )
    ...
    attractiveness_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="How attractive the tool is to attackers"
    )
    detection_score: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="How well it detects threats"
    )
    engagement_score: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="How much attackers engage with it"
    )
    ...
    last_triggered: Optional[datetime] = Field(
        default=None,
        description="Last time tool was triggered"
    )
...
class CatalogOptimizationConfig(BaseModel):
    ...
    min_tools: int = Field(
        default=5,
        ge=1,
        description="Minimum number of tools to maintain"
    )
    max_tools: int = Field(
        default=20,
        ge=1,
        description="Maximum number of tools to maintain"
    )
    ...
    min_score_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Minimum score to keep a tool"
    )
    ...
    promotion_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Score threshold to promote a tool"
    )
    
    evaluation_window_hours: int = Field(
        default=24,
        ge=1,
        description="Hours of data to consider for evaluation"
    )
    
    auto_retire_enabled: bool = Field(
        default=True,
        description="Automatically retire low-performing tools"
    )
    
    auto_generate_enabled: bool = Field(
        default=True,
        description="Automatically generate new tools"
    )
...
class ToolGenerationHint(BaseModel):
    ...
    # Observed patterns
    attempted_tools: List[str] = Field(
        description="Tools the attacker tried to use"
    )
    attack_categories: List[str] = Field(
        description="Attack categories observed"
    )
    ...
    confidence: float = Field(
        ge=0.0,
        le=1.0,
        description="Confidence in this hint"
    )
...
class AdaptiveToolRecommendation(BaseModel):
    ...
    tools_to_add: List[str] = Field(
        ...
        description="Tools to add to catalog"
    )
    tools_to_remove: List[str] = Field(
        default_factory=list,
        description="Tools to remove from catalog"
    )
    tools_to_modify: Dict[str, Dict[str, str]] = Field(
        default_factory=dict,
        description="Tools to modify (tool_name -> {field: new_value})"
    )
    ...
    expected_improvement: float = Field(
        ge=0.0,
        le=1.0,
        description="Expected improvement in effectiveness"
    )
    
    # Metadata
    based_on_sessions: List[str] = Field(
        description="Sessions used for analysis"
    )
...
class AttackerProfile(BaseModel):
    ...
    # Behavior patterns
    tool_preferences: List[str] = Field(
        description="Tools this attacker prefers"
    )
    attack_sequence: List[str] = Field(
        description="Typical attack sequence"
    )
    timing_pattern: str = Field(
        description="Attack timing: rapid, methodical, sporadic"
    )
    ...
    sophistication_score: float = Field(
        ge=0.0,
        le=1.0,
        description="Estimated sophistication level"
    )
    tool_diversity: int = Field(
        description="Number of different tools used"
    )
    uses_automation: bool = Field(
        default=False,
        description="Whether attacker appears to use automation"
    )
    
    # Personalization
    recommended_tools: List[str] = Field(
        description="Tools likely to attract this attacker"
    )
    bait_chain: List[str] = Field(
        default_factory=list,
        description="Sequence of tools to lead attacker deeper"
    )
...
class ABTestVariant(BaseModel):
    ...
    impressions: int = Field(
        default=0,
        description="Number of times shown to attackers"
    )
    triggers: int = Field(
        default=0,
    ...
    conversion_rate: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Trigger rate (triggers / impressions)"
    )
...
class ABTest(BaseModel):
    ...
    end_time: Optional[datetime] = Field(
        default=None,
        description="Test end time (None if ongoing)"
    )
    
    # Results
    winner: Optional[str] = Field(
        default=None,
        description="Winning variant ID (if test completed)"
    )
    ...
    status: str = Field(
        default="running",
        description="Test status: running, completed, cancelled"
    )
    min_sample_size: int = Field(
        default=100,
        description="Minimum samples needed per variant"
    )
...
class CatalogSnapshot(BaseModel):
    ...
    tool_metrics: Dict[str, ToolEffectivenessMetric] = Field(
    ...
    overall_effectiveness: float = Field(
        ge=0.0,
        le=1.0,
        description="Overall catalog effectiveness"
    )
    avg_tool_score: float = Field(
        ge=0.0,
        le=1.0,
        description="Average tool effectiveness score"
    )
    ...
    total_triggers: int = Field(
        description="Total tool triggers"
    )
