...
class AlertRule(BaseModel):
    ...
    min_threat_level: Optional[str] = Field(
        default=None,
        description="Minimum threat level (low, medium, high, critical)"
    )
    attack_categories: Optional[List[str]] = Field(
        default=None,
        description="Filter by attack categories"
    )
    pattern_types: Optional[List[str]] = Field(
        default=None,
        description="Filter by pattern types (coordinated, campaign, anomaly)"
    )
    min_confidence: Optional[float] = Field(
        default=None,
        ge=0.0,
        le=1.0,
        description="Minimum confidence score for patterns"
    )
    ...
    deduplicate_window_seconds: int = Field(
        default=300,
        description="Deduplication time window in seconds"
    )
    
    # Rate limiting
    rate_limit_count: Optional[int] = Field(
        default=None,
        description="Max alerts per time window"
    )
...
class Alert(BaseModel):
    ...
    pattern_id: Optional[str] = Field(
        default=None,
        description="Related pattern ID if applicable"
    )
    ...
    delivery_status: Dict[str, str] = Field(
        default_factory=dict,
        description="Delivery status per channel (pending, sent, failed)"
    )
