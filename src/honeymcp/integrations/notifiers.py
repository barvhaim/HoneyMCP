...
class NotifierBase(ABC):
    ...
    async def send_with_retry(self, alert: Alert) -> bool:
        ...
        while attempt < self.max_retries:
            try:
                ...
                if success:
                    ...
                    logger.info(
                        "Alert %s sent via %s on attempt %d",
                        alert.alert_id,
                        channel,
                        attempt + 1
                    )
        ...
        logger.error(
            "Alert %s failed via %s after %d attempts",
            alert.alert_id,
            channel,
            self.max_retries
        )
        ...
        if alert.event_id or alert.pattern_id:
            ...
            if alert.event_id:
                payload["payload"]["custom_details"]["links"].append({
                    "text": "View Event",
                    "href": f"/events/{alert.event_id}"
                })
            if alert.pattern_id:
                payload["payload"]["custom_details"]["links"].append({
                    "text": "View Pattern",
                    "href": f"/patterns/{alert.pattern_id}"
                })
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                for url in self.config.webhook_urls:
                    tasks.append(
                        self._send_to_webhook(session, url, payload)
                    )
...
class NotificationManager:
    ...
    async def send_alert(self, alert: Alert) -> bool:
        ...
        logger.info(
            "Alert %s sent: %d/%d channels successful",
            alert.alert_id,
            success_count,
            len(tasks)
        )
    ...
    async def send_alerts(self, alerts: list[Alert]) -> int:
        ...
        logger.info(
            "Batch send complete: %d/%d alerts successful",
            success_count,
            len(alerts)
        )
