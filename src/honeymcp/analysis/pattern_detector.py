...
class PatternDetector:
    ...
    async def detect_coordinated_attacks(
        ...
        for bucket_time, bucket_events in time_buckets.items():
            ...
            session_count_score = min(1.0, len(unique_sessions) / 10)
            tool_overlap_score = min(1.0, len(common_tools) / 5)
            ...
            confidence = (
                session_count_score * 0.4 +
                tool_overlap_score * 0.3 +
                time_cluster_score * 0.3
            )
    ...
    async def detect_attack_campaigns(
        ...
        for session_id, session_events_list in session_events.items():
            ...
            diversity_score = min(1.0, len(unique_tools) / 5)
            
            confidence = (
                duration_score * 0.4 +
                event_count_score * 0.3 +
                diversity_score * 0.3
            )
    ...
    async def detect_anomalies(
        ...
        if len(tool_counts) > 1:
            variance = sum((count - avg_usage) ** 2 for count in tool_counts.values()) / len(tool_counts)
            std_dev = variance ** 0.5
        else:
            std_dev = 0
        
        # Find tools with anomalous usage (>3 standard deviations)
        threshold = avg_usage + (3 * std_dev) if std_dev > 0 else avg_usage * 2
