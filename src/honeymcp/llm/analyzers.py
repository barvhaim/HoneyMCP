...
async def extract_tool_info(server: FastMCP) -> List[ToolInfo]:
    ...
    if hasattr(server, 'list_tools'):
        try:
            ...
            if tools:
                logger.info(f"Extracted {len(tools)} tools using list_tools method")
                return tools
        except Exception as e:
            logger.warning(f"Failed to extract tools using list_tools: {e}")
    ...
    if hasattr(server, '_tools'):
        try:
            ...
            if tools:
                logger.info(f"Extracted {len(tools)} tools using _tools dictionary")
                return tools
        except Exception as e:
            logger.warning(f"Failed to extract tools using _tools: {e}")
    ...
    if hasattr(server, '_docket') and hasattr(server._docket, 'tools'):
        try:
            ...
            if tools:
                logger.info(f"Extracted {len(tools)} tools using _docket")
                return tools
        except Exception as e:
            logger.warning(f"Failed to extract tools using _docket: {e}")
...
def categorize_tools(tools: List[ToolInfo]) -> Dict[str, List[ToolInfo]]:
    ...
    category_keywords = {
        'file_system': ['file', 'read', 'write', 'directory', 'path', 'folder', 'upload', 'download'],
        'database': ['database', 'query', 'sql', 'table', 'record', 'insert', 'update', 'delete'],
        'api': ['api', 'request', 'response', 'endpoint', 'http', 'rest', 'graphql'],
        'security': ['auth', 'token', 'credential', 'password', 'key', 'secret', 'permission'],
        'development': ['build', 'deploy', 'test', 'debug', 'compile', 'run', 'execute'],
        'communication': ['send', 'message', 'email', 'notify', 'alert', 'webhook'],
        'data_processing': ['process', 'transform', 'parse', 'convert', 'analyze', 'calculate']
    }
