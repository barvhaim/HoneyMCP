...
class ReportGenerator:
    ...
    def __init__(self) -> None:
        ...
        self._mitre_mappings = {
            ...
            "rce": {
                ...
                "techniques": ["T1059", "T1203", "T1569"],  # Command Interpreter, Exploitation, System Services
            },
            "privilege_escalation": {
                "tactics": ["TA0004"],  # Privilege Escalation
                "techniques": ["T1068", "T1078", "T1548"],  # Exploitation, Valid Accounts, Abuse Elevation
            },
            "credential_access": {
                "tactics": ["TA0006"],  # Credential Access
                "techniques": ["T1003", "T1110", "T1555"],  # Credential Dumping, Brute Force, Credentials from Password Stores
            },
            "discovery": {
                "tactics": ["TA0007"],  # Discovery
                "techniques": ["T1083", "T1087", "T1518"],  # File Discovery, Account Discovery, Software Discovery
            },
            "lateral_movement": {
                "tactics": ["TA0008"],  # Lateral Movement
                "techniques": ["T1021", "T1080", "T1550"],  # Remote Services, Taint Shared Content, Use Alternate Auth
            },
            "defense_evasion": {
                "tactics": ["TA0005"],  # Defense Evasion
                "techniques": ["T1027", "T1070", "T1562"],  # Obfuscation, Indicator Removal, Impair Defenses
            },
        }
    ...
    def _generate_summary(self, timeline: AttackTimeline) -> str:
        ...
        if timeline.event_count >= 10:
            ...
            summary_parts.append("High sophistication - diverse tool usage indicates skilled attacker.")
    ...
    def _identify_attack_vector(self, timeline: AttackTimeline) -> str:
        ...
        elif any("credential" in tool.lower() or "password" in tool.lower() for tool in first_tools):
    ...
    def _extract_iocs(self, timeline: AttackTimeline) -> List[str]:
        ...
        if timeline.avg_time_between_events < 5:
            iocs.append(f"Rapid attack sequence: {timeline.avg_time_between_events:.1f}s between attempts")
    ...
    def _generate_comparison_analysis(
        ...
        if common_categories:
            parts.append(
                f"Common attack categories: {', '.join(common_categories)}."
            )
